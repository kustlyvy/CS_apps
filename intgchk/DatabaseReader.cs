    ...
    class DatabaseReader
    {
        ...
        public static void OpenConnection(Parameters prm)
        {
            string conn = String.Format(
                @"user id={0};password={1};server={2};Trusted_Connection=no;database={3};connection timeout=100000",
                prm.login, 
                prm.password, 
                prm.server, 
                prm.database
       ...
       /* public static void ReadClientPractices(ref SqlDataReader reader)
        ...
        public static void ReadChargesByADSRecord(/*DateTime dt, List<string> rec*/Dictionary<DateTime, string> postDates, ref SqlDataReader reader)
        {
            // postDates.First().Key

            /* string qry = @"SELECT dbo.Patients.[LastName], 
                                        dbo.Patients.FirstName, 
                                        dbo.Patients.AccountId, 
                                        dbo.Charges.DateOfService, 
                                        dbo.Charges.DateOfPosting,
                                        dbo.Charges.Billed,
                                        dbo.Charges.Payment,
                                        dbo.Charges.Adjustment,
                                        dbo.Charges.Balance
                            FROM [CollectionSystem-Imports].dbo.Charges INNER JOIN dbo.Patients on dbo.Charges.PatientId = dbo.Patients.ID  
                            where dbo.Charges.CreateDate > '" + dt.ToString() + "' and dbo.Patients.AccountId='" + rec[2] + "'";
                            */
            ...
            string qry = @"SELECT dbo.Patients.[LastName], 
                                       ...
                                       dbo.Charges.Balance
                           FROM [CollectionSystem-Imports].dbo.Charges INNER JOIN dbo.Patients on dbo.Charges.PatientId = dbo.Patients.ID  
                           where dbo.Charges.toDelete<>1 and dbo.Charges.DateOfPosting >= '" + postDates.First().Key.ToString() + "' and dbo.Charges.DateOfPosting <= '" + postDates.Last().Key.ToString() + "' and dbo.Patients.AccountId='" + postDates.First().Value.ToString() + "'";

            ReadDataByCommand(qry, ref reader);
        }


        public static void ReadChargesByADSDetailed(List<string> rec, ref SqlDataReader reader)
        {
            string qry = @"SELECT dbo.Patients.[LastName], 
                                       dbo.Patients.FirstName, 
                                       dbo.Patients.AccountId, 
                                       ...
                                       dbo.Charges.Balance
                           FROM [CollectionSystem-Imports].dbo.Charges INNER JOIN dbo.Patients on dbo.Charges.PatientId = dbo.Patients.ID  
                           where dbo.Charges.toDelete<>1 and dbo.Charges.DateOfService = '" + rec[4] +
                           "' and dbo.Charges.DateOfPosting = '" + rec[3] +
                           "' and dbo.Patients.AccountId='" + rec[2] + "'";

            ReadDataByCommand(qry, //not in (select id from [CollectionSystem-PreRelease].dbo.CPTs)",
               ref reader
);
        }
...
insert into @PreTbl select id, LiveID from [CollectionSystem-Imports].dbo.charges  where updated = 1 and liveID > 0 and toDelete<>1;
      ...
      */
        ...
        public static void GetChargesWithNullLiveId(ref SqlDataReader reader)
        {
            ...
            ReadDataByCommand(@"SELECT Id,      
                          ...
                          where toDelete<>0 and (LiveID is NULL)",
                          ref reader
             );
